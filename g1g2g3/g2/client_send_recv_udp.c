...
int main(int argc, char *argv[]){
  ...
  while(1){
    ...
    if(n_read == 0) {
      char EOD[N];
      for(i = 0; i < N; i++) EOD[i] = '1';
      int n_send2 = sendto(s, EOD, 1, 0, (const struct sockaddr *)&addr, sizeof(addr));
      if(n_send2 == -1){perror("n_send2"); exit(1);}
      break;//実質こっちがさきにブレイクしてくれる
    }
    int n_send = sendto(s, data2, 1, 0, (const struct sockaddr *)&addr, sizeof(addr));
