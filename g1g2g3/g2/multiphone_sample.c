...
int main(int argc, char *argv[]){
  ...
  if(argc == 4){perror("argument"); exit(1);}
  time_t t1;  
  time(&t1);
  ...
  if((myhost = gethostbyname(hostname)) == NULL){perror("gethostbyname"); exit(1);}
  ...
  bcopy(muhost->h_addr, (char *)&serv_addr.sin_addr, myhost->h_length);
  if(bind(ss, (struct sockaddr *)&serv_addr, sizeof(serv_addr)) == -1){perror("bind"); exit(1);}
